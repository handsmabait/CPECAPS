...
\emph{\citet{venkatrayappa2024survey3dhumanbody}} & Evaluates 3D human body pose and shape estimation methods for contemporary dance & Comparative survey of model families: SMPL(-A/X), MANO, STAR, FLAME (optimization-based) and HMR, VIBE, SPIN, PARE, EXPOSE, PHALP (learning-based); analysis by input modality (single-image, multi-view, video). & PHALP strong overall; SMPL-X improves expressiveness; STAR excels in temporal modeling. Common limits: occlusion, lighting, and compute needs. \\ \hline

\emph{\citet{Protopapadakis2018dancepose}} & Identifies dance types from motion-capture skeletal data & Kinect skeletal features with PCA and frame differencing; compared classifiers (k-NN, Naïve Bayes, LDA/DA, decision trees, Random Forest, SVM, ensembles). & k-NN and Random Forest performed best; multimodal or multi-sensor data recommended to handle occlusion. \\ \hline

\emph{\citet{ZHAO202566}} & Real-time pose estimation for complex dances & Hybrid architecture: Vision Transformer + Time-Series Transformer trained on AIST and DanceTrack datasets. & MPJPE: 18.4 mm / 20.1 mm; MOTA: 92.3\% / 89.5\%; latency \(\approx\) 35.2 ms (real-time capable). \\ \hline

\emph{\citet{leililiu2023dancemovement}} & Improves recognition accuracy for traditional dance movements & Keypoint detection via Part Affinity Fields (PAFs); temporal modeling with LSTM classifiers. & \(>\)85\% overall accuracy; 95.2\% for curtsey movements. \\ \hline

\emph{\citet{zheng2023deeplearningbasedhumanpose}} & Deep-learning approaches for pose design and recognition & Backbone fusion (ResNet-152 + HR-Net) with global–local feature fusion and class-imbalance handling strategies. & Reported metrics: accuracy 0.9870; precision 0.9851; sensitivity 0.9873; F1 0.9861; Kappa 0.9841. \\ \hline

\emph{\citet{xu_adaptive_2022}} & Multi-person pose estimation from single images & Two-stage Adaptive Hypergraph Neural Network (keypoint localization + adaptive hypergraph) with SIC module; end-to-end training. & Achieves state-of-the-art performance on MS-COCO, MPII, and CrowdPose benchmarks. \\ \hline

\emph{\citet{tolgyessy2021skeletontracking}} & Quantifies joint-level accuracy and repeatability across Kinect sensors & Controlled robotic-manipulator and figurine measurements across positions; compared Kinect v1, v2, Azure Kinect (NFOV/WFOV). & Azure NFOV shows highest accuracy (0.8--1.9 mm SD); joint failures 15--30\% under occlusion; performance declines at long ranges. \\ \hline

\emph{\citet{linjustdance}} & Effects of feedback and controller use in dance exergames & 2\(\times\)2\(\times\)2 factorial experimental design (feedback \(\times\) controller \(\times\) sex); 129 participants; 12-minute sessions. & Mean HR \(\approx\) 109 bpm; immediate/clear feedback increased engagement and perceived competence. \\ \hline

\emph{\citet{yuxiong2019}} & DTW-based scoring for rehabilitation/exercise movements & Dynamic Time Warping on 8 bone vectors + body orientation; algorithm converting DTW distance to a 0--100\% performance score. & Scores correlated strongly with expert ratings (\(r \approx 0.86\)); robust to tempo variation and some occlusion. \\ \hline

\emph{\citet{trallis2019}} & Choreographic pattern analysis from heterogeneous capture systems & Trajectory-based DTW similarity for choreography; sensor comparison (VICON vs Kinect); experiments on smoothing and joint selection. & Kinect is noisier but DTW reduces sensor bias; smoothing and selective joint use improve retrieval accuracy. \\ \hline

\emph{\citet{sun2025dancemovement}} & Pose estimation in complex dance scenes & Enhanced HRNet backbone with improved feature extraction and robustness modules for cluttered scenes. & Improved keypoint accuracy and robustness under occlusion/clutter. \\ \hline

\emph{\citet{Erdem2025}} & Deep-learning--based scoring for Turkish folk dance & Webcam capture; pose extraction via MediaPipe/YOLO; comparative models: DTW, TLCC, LSTM, Siamese networks. & LSTM produced higher scores ($\approx$ 68.43, MSE $\approx$ 56.11) vs DTW ($\approx$ 60.64, MSE $\approx$ 139.32); system is sensitive to camera angle. \\ \hline
